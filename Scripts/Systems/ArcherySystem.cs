...
public partial class ArcherySystem : Node
{
	...
	public override void _Ready()
	{
		...
		// [FIX] Initialize SpawnPosition from Scene (with global search)
		...
		if (spawnPoint != null)
		{
			...
			foundSpawn = true;
			GD.Print($"ArcherySystem: SpawnPosition set to {SpawnPosition} from {spawnPoint.Name}");
		}
		...
		if (MobaGameManager.Instance != null && _currentPlayer != null)
		{
			...
			if (teamSpawn != null)
			{
				SpawnPosition = teamSpawn.GlobalPosition;
				foundSpawn = true;
				GD.Print($"ArcherySystem: Spawning at Team Spawn: {teamSpawn.Name} ({SpawnPosition})");
			}
			else
			{
				...
				if (nexus != null)
				{
					SpawnPosition = nexus.GlobalPosition + Vector3.Up;
					foundSpawn = true;
		...
		{
			_currentPlayer.TeleportTo(SpawnPosition, SpawnPosition + Vector3.Forward * 10.0f);
		}
