...
public partial class NetworkManager : Node
{
    ...
    public void SpawnPlayer(long id, MobaTeam team, string className = "Warrior")
    {
        ...
        GD.Print($"[NetworkManager] DEBUG: SpawnPlayer(id={id}, team={team}, class={className})");

        // 1. Try to find a team-specific spawn point FIRST
        Node3D spawnPoint = null;
        string teamSpawnName = $"SpawnPoint_{team.ToString()}";
        GD.Print($"[NetworkManager] DEBUG: Searching for spawn point: {teamSpawnName}");
        spawnPoint = GetTree().CurrentScene.FindChild(teamSpawnName, true, false) as Node3D;

        if (spawnPoint == null)
        {
            // 2. Fallback to standard generic names
        ...
        if (spawnPoint != null)
        {
            // Increased range for spread (3m vs 1m) to avoid player collisions
            ...
            GD.Print($"[NetworkManager] SUCCESS: Found {spawnPoint.Name} at {spawnPos}. Team: {team}");
        }
        else
        {
            GD.Print($"[NetworkManager] WARNING: No spawn point found for {teamSpawnName}. Falling back to MobaGameManager locations.");
            // FALLBACK: Team-based spawn at creep locations from MobaGameManager
            if (MobaGameManager.Instance != null)
            {
                // If team is None, default to Red (the left/bottom side) for host/orphans
                ...
                if (teamSpawn != Vector3.Zero)
                {
                    ...
                    GD.Print($"[NetworkManager] SUCCESS: Spawning at {finalTeam} creep location: {teamSpawn}");
                }
                else
                {
                    GD.PrintErr($"[NetworkManager] CRITICAL: MobaGameManager {finalTeam} spawn pos is Zero! Spawning at origin.");
                }
            }
            else
            {
                GD.PrintErr("[NetworkManager] CRITICAL: No spawn point found AND MobaGameManager missing! Spawning at origin.");
            }
        }
