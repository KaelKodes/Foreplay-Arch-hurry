...
public partial class ObjectPlacer : Node
{
    ...
    public void ConfirmPlacement()
    {
            ...
            (netManager.Multiplayer.IsServer() || netManager.Multiplayer.GetUniqueId() != 1))
        {
            ...
            else if (!string.IsNullOrEmpty(_currentObject.ScenePath))
            {
                // Use ScenePath if set (this is the original .tscn scene for spawning)
                resourcePath = _currentObject.ScenePath;
            }
            else if (!string.IsNullOrEmpty(_currentObject.ModelPath))
            {
                // Fallback to ModelPath if ScenePath not set (raw GLTF/FBX models)
                resourcePath = _currentObject.ModelPath;
            }
            else if (objName == "Monsters" || _currentObject is Monsters)
            {
                string species = (_currentObject as Monsters)?.Species ?? "Yeti";
                resourcePath = $"res://Scenes/Entities/Monsters.tscn:{species}";
            }
            else
            {
