...
/// Extracted from Monster.cs to reduce file size.
/// </summary>
public static class MonsterVisuals
{
    ...
    /// </summary>
    public static void UpdateSpeciesVisuals(Node monster, string species, AnimationPlayer animPlayer, Action onCollisionUpdate)
    {
    ...
    /// </summary>
    public static void UpdateCollisionShape(Node monster)
    {
    ...
    private static void ShowSpeciesRecursive(Node node, string species, ref bool found, AnimationPlayer animPlayer)
    {
        foreach (Node child in node.GetChildren())
        {
            ...
            if (isMatch)
            {
                if (!found) GD.Print($"  [MonsterVisuals] MATCH FOUND: {name} for {species}");
                found = true;
                ShowAncestors(child);
