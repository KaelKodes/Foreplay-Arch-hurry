...
public partial class MainHUDController : CanvasLayer
{
	...
	private void SelectObjectToPlace(string objectId)
	{
        ...
        else
        {
            ...
            {
                scenePath = "res://Scenes/Entities/Monsters.tscn";
            }
            else
            {
                switch (objectId)
        ...
        else
        {
			...
			if (instance is not InteractableObject interactable)
			{
				...
				wrapper.ScenePath = scenePath; // Track scene for network spawning
			...
			else
			{
				obj = interactable;
				obj.ObjectName = objectId;
				obj.ScenePath = scenePath; // Always set scene path for network spawning
										   // Only set ModelPath if not already defined (scene may have FBX path for texturing)
				if (string.IsNullOrEmpty(obj.ModelPath))
					obj.ModelPath = scenePath;
				if (obj is Monsters monster)
				{
